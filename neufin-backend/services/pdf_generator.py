...
def _make_styles(bc: BrandColors):
    ...
    return {
        "cover_title": ps("CoverTitle",
            fontSize=32, textColor=bc.white, fontName="Helvetica-Bold",
            alignment=TA_CENTER, leading=40, spaceAfter=8),
        "cover_sub": ps("CoverSub",
            fontSize=14, textColor=_hex("#93C5FD"), alignment=TA_CENTER, spaceAfter=6),
        "cover_meta": ps("CoverMeta",
            fontSize=11, textColor=_hex("#D1D5DB"), alignment=TA_CENTER),
        "h1": ps("H1",
            fontSize=18, textColor=bc.primary, fontName="Helvetica-Bold",
            spaceBefore=14, spaceAfter=8),
        "h2": ps("H2",
            fontSize=13, textColor=bc.text, fontName="Helvetica-Bold",
            spaceBefore=10, spaceAfter=5),
        "body": ps("Body",
            fontSize=10, textColor=bc.text, leading=16),
        "body_muted": ps("BodyMuted",
            fontSize=9, textColor=bc.muted, leading=14),
        "bullet": ps("Bullet",
            fontSize=10, textColor=bc.text, leading=16,
            leftIndent=14, bulletIndent=6, spaceBefore=3),
        "score_big": ps("ScoreBig",
            fontSize=48, fontName="Helvetica-Bold", textColor=bc.primary,
            alignment=TA_CENTER),
        "center": ps("Center",
            fontSize=10, textColor=bc.text, alignment=TA_CENTER),
        "footer": ps("Footer",
            fontSize=7.5, textColor=bc.muted, alignment=TA_CENTER),
        "toc": ps("TOC",
            fontSize=11, textColor=bc.text, leading=20),
    }
...
def _bar_row(label: str, pct: float, bc: BrandColors, bar_width: float = 3.5) -> Table:
    ...
    return Table(
        [[Paragraph(label, ParagraphStyle("bl", fontSize=9, textColor=bc.text)),
          inner,
          Paragraph(f"{pct:.1f}%", ParagraphStyle("br", fontSize=9, textColor=bc.muted, alignment=TA_RIGHT))]],
...
def _make_page_callbacks(bc: BrandColors, advisor_name: str, report_date: str, logo_img=None):
    def header(canvas, doc):
        ...
        if logo_img:
            try:
                canvas.drawImage(logo_img, 0.5 * inch, 10.45 * inch, width=1.2 * inch, height=0.38 * inch,
...
def generate_advisor_report(
    ...
    header_cb, footer_cb, cover_footer_cb = _make_page_callbacks(bc, advisor_name, report_date, logo_img)
    ...
    doc.addPageTemplates([
        ...
        PageTemplate(id="Body",  frames=[body_frame],  onPage=lambda c, d: (header_cb(c, d), footer_cb(c, d))),
    ])
    ...
    cover_bg = Table(
        ...
        colWidths=[7.7 * inch], rowHeights=[1.6 * inch],
    )
    ...
    E.append(PageBreak())
    E.append(Paragraph("__SWITCH_TEMPLATE__", ParagraphStyle("_", fontSize=0.1)))  # trigger handled below
    ...
    summary_rows = [
        ...
        ["Total Value",        f"${metrics.get('total_value', 0):,.0f}",
         "Positions",          str(metrics.get("num_positions", 0))],
        ["Max Position",       f"{metrics.get('max_position_pct', 0):.1f}%",
         "Concentration Risk", f"{metrics.get('concentration_risk', 0):.1f}"],
        ["Diversification",    f"{metrics.get('diversification_score', 0)}/30",
         "Annualised Vol",     f"{metrics.get('annualized_volatility', 0):.1f}%"],
    ]
    ...
    if positions:
        ...
        E.append(_colored_table(pos_rows, [1.1*inch, 1.2*inch, 1.3*inch, 1.5*inch, 1.1*inch], bc))
    else:
    ...
    E.append(Paragraph(
        ...
        + (f"The unrealised return stands at <b>{pnl:+.2f}%</b> against cost basis." if pnl is not None
    ...
    perf_rows.append(["Max Position Weight", f"{metrics.get('max_position_pct', 0):.1f}%",
    ...
    E.append(HRFlowable(width="100%", thickness=0.8, color=bc.primary))
    E.append(Spacer(1, 10))
    risk_text = analysis.get("risk_assessment",
    ...
    SECTOR_MAP = {
        ...
        "AMD": "Technology", "TSLA": "Consumer Disc.", "AMZN": "Consumer Disc.",
        "NFLX": "Comm. Services", "JPM": "Financials", "BAC": "Financials",
        "GS": "Financials", "MS": "Financials", "V": "Financials",
        "MA": "Financials", "JNJ": "Healthcare", "PFE": "Healthcare",
        "UNH": "Healthcare", "MRK": "Healthcare", "XOM": "Energy",
        "CVX": "Energy", "COP": "Energy", "WMT": "Consumer Staples",
    ...
    for i, strength in enumerate(analysis.get("strengths", []), 1):
        blk = Table(
            [[Paragraph(f"<b>{i}</b>", S["center"]), Paragraph(f"<b>{strength}</b>", S["body"])]],
            colWidths=[0.4*inch, 6.8*inch], rowHeights=[0.36*inch],
        )
    ...
    E.append(HRFlowable(width="100%", thickness=0.8, color=bc.primary))
    E.append(Spacer(1, 10))
    E.append(Paragraph("Areas requiring attention or presenting opportunities for improvement:", S["body"]))
    E.append(Spacer(1, 12))
    ...
    for i, action in enumerate(analysis.get("action_items", []), 1):
        row = Table(
            ...
            colWidths=[0.4*inch, 6.8*inch], rowHeights=[0.34*inch],
        )
