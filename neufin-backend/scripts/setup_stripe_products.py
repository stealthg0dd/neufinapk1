...
def main():
    ...
    print(f"  STRIPE_PRICE_ADVISOR_REPORT_ONETIME={results['advisor_report_onetime']['price_id']}")
    print()
    print("Update routers/vault.py PLANS constant stripe_price_id values:")
    print()
    print(
        f"  retail.stripe_price_id      = \"{results['retail']['price_id']}\""
    )
    print(
        f"  advisor.stripe_price_id     = \"{results['advisor']['price_id']}\""
    )
    print(
        f"  enterprise.stripe_price_id  = \"{results['enterprise']['price_id']}\""
    )
    print()
