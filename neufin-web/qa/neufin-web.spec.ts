...
test.describe("NEUFIN WEB — A1–A5 Health Check", () => {
  ...
  test("A2 — Google OAuth redirect reaches accounts.google.com", async ({
    page,
  }) => {
  ...
  test("A4 — Pricing page tiers + disclaimer present", async ({ page }) => {
    ...
    await expect(
      page.locator("p", { hasText: /^Free$/ }).first(),
    ).toBeVisible();
    await expect(
      page.locator("p", { hasText: /^Advisor$/ }).first(),
    ).toBeVisible();
    await expect(
      page.locator("p", { hasText: /^\$299$/ }).first(),
    ).toBeVisible();
    await expect(
      page.locator("p", { hasText: /^Enterprise$/ }).first(),
    ).toBeVisible();
    await expect(
      page.locator("p", { hasText: /^\$999$/ }).first(),
    ).toBeVisible();
