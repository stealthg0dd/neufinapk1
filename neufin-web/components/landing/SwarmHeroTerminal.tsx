...
const AGENTS: AgentEntry[] = [
  {
    name: "MARKET REGIME",
    status: "complete",
    output: "Risk-Off detected · VIX 28.4 · Yield curve inverted",
  },
  {
    name: "STRATEGIST",
    status: "complete",
    output: "Defensive positioning recommended · Macro headwinds",
  },
  {
    name: "QUANT",
    status: "complete",
    output: "HHI 0.34 · Beta 0.82 · Sharpe 1.24 · 3 clusters",
  },
  {
    name: "TAX ARCHITECT",
    status: "complete",
    output: "CGT exposure $4,200 · 3 positions at risk",
  },
  {
    name: "RISK SENTINEL",
    status: "complete",
    output: "Concentration risk HIGH · Tech cluster 67%",
  },
  {
    name: "ALPHA SCOUT",
    status: "complete",
    output: "2 opportunities identified · Defensive rotation play",
  },
  {
    name: "SYNTHESIZER",
    status: "running",
    output: "Generating IC briefing...",
  },
];

function StatusIcon({ status }: { status: AgentStatus }) {
  if (status === "complete")
    return <CheckCircle className="mt-0.5 h-3 w-3 shrink-0 text-positive" />;
  if (status === "running")
    return (
        ...
        {entries.map((a, idx) => {
          ...
          return (
            ...
            >
              ...
              <span className="w-28 shrink-0 text-sm font-bold uppercase text-primary">
                {a.name}
              </span>
        ...
        {showResult ? (
          <div className="mt-3 rounded-lg border border-primary/30 bg-primary/10 p-3">
            <p className="mb-1 font-mono text-sm uppercase tracking-widest text-primary">
              IC BRIEFING GENERATED
            </p>
            <p className="text-sm font-semibold text-slate-100">
              DNA Score{" "}
              <span className="font-mono tabular-nums text-primary">78</span> ·
              Investor Type{" "}
              <span className="text-slate-100">Defensive Allocator</span>
            </p>
            <p className="mt-1 text-sm text-warning">
              Overconfidence bias detected in 3 positions
            </p>
          </div>
        ) : null}
      </div>
