...
function statusBadge(
  badge: 'DONE' | 'RUNNING' | 'FAILED' | 'WAITING',
): { node: React.ReactNode; className: string } {
  if (badge === 'DONE') {
    return {
      node: <CheckCircle className="h-3.5 w-3.5 text-[#16A34A]" strokeWidth={2} aria-label="Complete" />,
      className: 'flex items-center justify-center rounded border border-emerald-200 bg-emerald-50 px-2 py-0.5',
    }
  }
  const label = badge === 'RUNNING' ? 'Running' : badge === 'FAILED' ? 'Failed' : 'Waiting'
  const badgeClass =
    badge === 'RUNNING'
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-[#E5E7EB] bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-[#E5E7EB] bg-[#F8FAFC] px-4 py-3">
        <div className="text-section-title pr-3">Portfolio Intelligence — Agent Analysis</div>
