...
function ProviderTooltip({ name, stats, rl, onClose }: TooltipProps) {
  ...
  return (
    <div
      ref={ref}
      className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 w-48 rounded-xl border border-gray-700 bg-gray-900 p-3 shadow-2xl"
    >
      {/* Arrow */}
      ...
      <p className="text-xs font-semibold text-gray-100 mb-2.5">{providerLabel(name)}</p>
      ...
      <div className="grid grid-cols-2 gap-x-2 gap-y-1.5 text-[10px]">
        ...
        <span className="text-gray-300 tabular-nums text-right">${costToday.toFixed(4)}</span>

        <span className="text-gray-500">Total calls</span>
        <span className="text-gray-300 tabular-nums text-right">{stats.total_requests.toLocaleString()}</span>

        <span className="text-gray-500">Failures</span>
        <span className={`tabular-nums text-right ${stats.total_failures > 0 ? "text-red-400" : "text-gray-300"}`}>
      ...
      {stats.last_failure_reason && (
        <p className="mt-2 text-[9px] text-red-400/70 line-clamp-2">{stats.last_failure_reason}</p>
      )}
    </div>
  )
}
