...
function ProviderTooltip({ name, stats, rl, onClose }: TooltipProps) {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs font-semibold text-shell-fg mb-2.5">
        {providerLabel(name)}
      </p>
      ...
      <div className="grid grid-cols-2 gap-x-2 gap-y-1.5 text-sm">
        ...
        <span
          className={`tabular-nums text-right ${stats.total_failures > 0 ? "text-red-400" : "text-shell-fg/90"}`}
        >
      ...
      {stats.last_failure_reason && (
        <p className="mt-2 text-sm text-red-400/70 line-clamp-2">
          {stats.last_failure_reason}
        </p>
      )}
    </div>
  ...
  return (
    <div className="relative flex-shrink-0">
      ...
      >
        {/* Status dot */}
        <span
          className={`h-1.5 w-1.5 flex-shrink-0 rounded-full ${dotClass(stats)}`}
        />
