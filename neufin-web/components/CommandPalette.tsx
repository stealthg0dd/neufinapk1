...
const SUGGESTIONS = [
  ...
  {
    label: "How does inflation affect my holdings?",
    icon: "🌐",
    category: "Macro",
  },
  ...
  {
    label: "Optimize my largest position for tax efficiency",
    icon: "💡",
    category: "Tax",
  },
  {
    label: "Which positions have the highest beta?",
    icon: "⚡",
    category: "Quant",
  },
...
function classifyQuestion(msg: string): string {
  ...
  if (/risk|corr|beta|sharpe|volatil|hhi|concentration|calcul/.test(m))
    return "quant";
  if (/news|macro|inflation|cpi|market|econom|fed|rate|cat\b|sector/.test(m))
    return "macro";
  return "synthesizer";
}
      ...
      >
        <div className="cp-panel w-full max-w-2xl bg-[#0D0D0D] border border-[#333] rounded-md shadow-2xl font-mono overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-[#222]">
            <span className="text-[#FFB900] text-sm select-none shrink-0">
              $
            </span>
            <input
              ref={inputRef}
            ...
            {agentMeta && !loading && (
              <span
                ...
                style={{
                  ...
                  borderColor: `${agentMeta.color}40`,
                }}
              >
                → {agentMeta.label}
              </span>
            ...
            <div className="px-4 py-3 border-b border-[#1e1e1e] space-y-3">
              ...
              {Object.keys(result.response?.key_numbers ?? {}).length > 0 && (
                <div className="flex flex-wrap gap-3">
                  {Object.entries(result.response?.key_numbers ?? {}).map(
                    ([k, v]) => (
          ...
          {!result && !loading && (
            <div className="cp-scroll max-h-72 overflow-y-auto py-1">
              {filtered.length === 0 && (
                <div className="px-4 py-3 text-sm text-[#444]">
              ...
              {filtered.map((s, idx) => (
                ...
                >
                  <span className="text-[14px] shrink-0 w-5 text-center">
                    {s.icon}
                  </span>
                  <span className="flex-1 text-sm text-[#C8C8C8]">
                    {s.label}
                  </span>
                  <span
                    ...
                    style={{
                      color:
                        AGENT_META[s.category.toLowerCase()]?.color ?? "#666",
          ...
          <div className="px-4 py-1.5 bg-[#111] border-t border-[#1e1e1e] flex items-center gap-4 text-sm text-[#444] select-none">
            <span>
              <kbd className="border border-[#333] rounded px-1">↑↓</kbd>{" "}
              navigate
            </span>
            <span>
              <kbd className="border border-[#333] rounded px-1">⏎</kbd> send
            </span>
            <span>
              <kbd className="border border-[#333] rounded px-1">ESC</kbd> close
            </span>
