  ...
  return (
    <div className="mt-3 rounded-xl border border-shell-border/50 bg-shell/80 p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold uppercase tracking-widest text-shell-muted">
          Full Brief — {venture.name}
        </span>
        <button
          onClick={onClose}
          className="text-shell-subtle hover:text-shell-fg/90 transition-colors text-xs"
        >
          collapse ↑
        </button>
      </div>
      ...
      {/* Full brief */}
      {venture.briefFull ? (
        <pre className="whitespace-pre-wrap font-mono text-xs text-shell-fg/90 leading-relaxed max-h-64 overflow-y-auto scrollbar-thin">
          {venture.briefFull}
        </pre>
      ) : (
      ...
      {(blocked.length > 0 || pending.length > 0) && (
        <div className="space-y-1.5">
          <p className="text-sm font-semibold uppercase tracking-widest text-shell-subtle">
...
export default function VentureCard({
  ...
  return (
    <div className="flex-shrink-0 w-64 sm:w-auto sm:flex-1 min-w-56">
      ...
      <button
        onClick={onToggle}
        className={`w-full text-left rounded-xl border bg-shell/70 backdrop-blur-sm p-4 transition-all duration-200 ${colors.border} ${colors.glow} ${
          expanded ? "ring-1 ring-white/10 bg-shell" : ""
        }`}
      >
        ...
        {/* Brief excerpt */}
        <p className="text-xs text-shell-muted leading-relaxed line-clamp-2 mb-3 min-h-[2.5rem]">
          {venture.briefExcerpt || (
            <span className="text-shell-subtle italic">No brief today</span>
          )}
        </p>
        ...
        <div className="flex items-center gap-2">
          ...
          {hasBlocked && (
            <span className="rounded-full bg-red-500/20 border border-red-500/30 px-2 py-0.5 text-sm font-semibold text-red-300">
              {venture.taskCounts.blocked} blocked
            </span>
          )}
          {!hasBlocked && venture.taskCounts.blocked === 0 && (
            <span className="rounded-full bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 text-sm text-emerald-400">
              clear
            </span>
          )}
          {/* Expand chevron */}
