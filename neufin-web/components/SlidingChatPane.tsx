...
export function SlidingChatPane({
  ...
  const send = async (question: string) => {
    ...
    setMessages((prev) => [...prev, { role: 'assistant', content: '...', agent: 'MD', streaming: false }])
    ...
    try {
      ...
      const reply = (data.reply as string) ?? ((data.response as Record<string, string>)?.answer ?? 'Analysis complete.')
      const keyNumbers =
        (data.key_numbers as KeyNumbers) ?? ((data.response as Record<string, KeyNumbers>)?.key_numbers ?? {})
      const action =
          ...
          <div className="flex flex-col gap-4">
            ...
            {loading && (
              <div className="flex items-center gap-2 text-[9px]" style={{ fontFamily: MONO, color: TRANS_MUTED }}>
                ...
                <span className="animate-pulse text-primary-dark">Reviewing analysis…</span>
              </div>
            )}
          </div>
        </div>
