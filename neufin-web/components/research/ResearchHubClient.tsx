    ...
    <div className="min-h-screen bg-app text-navy">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12 space-y-12">
        ...
        <form onSubmit={runSearch} className="space-y-4">
          ...
          {searchErr && (
            <GlassCard className="border-danger2/30 p-4 text-sm text-danger2">{searchErr}</GlassCard>
          )}
          {searchHits.length > 0 && (
            <div className="space-y-2">
              ...
              {searchHits.map((h) => (
                <GlassCard key={h.id || h.title} className="p-4">
                  {h.title && <p className="mb-1 font-medium text-navy">{highlightText(h.title, q)}</p>}
                  {h.summary && (
                    <p className="line-clamp-3 text-sm text-slate2">{highlightText(h.summary, q)}</p>
                  )}
                  {h.similarity != null && (
        ...
        <section>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {notes.map((note) => {
              ...
              return (
                <GlassCard key={note.id} className="p-5 flex flex-col h-full border-l-2 border-l-primary/60">
                  ...
                  <h3 className="mb-2 text-lg font-semibold text-navy">{note.title}</h3>
                  <p className="line-clamp-3 flex-1 text-sm text-slate2">{note.executive_summary}</p>
                  ...
                  <div className="mt-4 flex items-center justify-between gap-2 flex-wrap">
                    {locked ? (
                      ...
                      <Link href={`/research/${note.id}`} className="text-sm text-primary font-medium hover:underline">
                        Read full report →
                      </Link>
                    )}
                  </div>
