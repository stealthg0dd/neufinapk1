...
export default function PublicResearchHubClient({
  notes,
}: {
  notes: BlogNote[];
}) {
  ...
  const filtered = useMemo(() => {
    ...
    if (tab === "BEHAVIORAL")
      return notes.filter((n) =>
        n.note_type.toLowerCase().includes("behavior"),
      );
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {current.map((note) => (
          ...
          >
            <span
              className={`rounded-full border px-2 py-0.5 text-sm font-mono ${typeCls(note.note_type.toLowerCase())}`}
            >
              {fmtType(note.note_type)}
            </span>
            <h3 className="mb-2 mt-2 text-base font-semibold leading-snug text-foreground">
              {note.title}
            </h3>
            ...
            <div className="flex items-center justify-between gap-2 text-sm">
              ...
              <span className="font-mono text-primary">
                {Math.round((note.confidence_score ?? 0) * 100)}% confidence
              </span>
            </div>
            <Link
