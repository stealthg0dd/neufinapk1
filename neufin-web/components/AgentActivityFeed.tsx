  ...
  return (
    <div className="space-y-2">
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="flex items-center gap-3 rounded-lg bg-shell-raised/40 px-3 py-2.5 animate-pulse"
        >
      ...
      {visible.map((log, i) => {
        ...
        return (
          ...
          >
            ...
            {/* Cost */}
            <span className="text-shell-subtle tabular-nums flex-shrink-0">
              {cost}
            </span>

            {/* Latency */}
            {latency && (
              <span className="text-shell-subtle tabular-nums flex-shrink-0 hidden md:block">
                {latency}
              </span>
            )}
            ...
            {/* Provider */}
            <span
              className={`flex-shrink-0 rounded px-1.5 py-0.5 text-sm font-medium ${
                log.success === false
