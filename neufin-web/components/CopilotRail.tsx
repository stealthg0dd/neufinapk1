...
export function CopilotRail({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <aside
      aria-hidden={!open}
      className={`fixed right-0 top-11 bottom-0 z-40 flex w-80 flex-col border-l border-border/60 bg-[hsl(var(--copilot))] transition-transform duration-300 ease-in-out ${
    ...
    >
      ...
      <div className="shrink-0 border-b border-border/40 px-4 py-2.5">
        ...
        <p className="mt-1.5 font-mono text-[10px] text-muted-foreground/60">7 agents active</p>
      </div>
      ...
      <div className="shrink-0 border-t border-border/40 px-4 py-3">
        <p className="mb-2 font-mono text-[9px] uppercase text-muted-foreground/50">Quick prompts</p>
        <div className="flex flex-wrap gap-2">
          {QUICK_PROMPTS.map((label) => (
            <button
              ...
              className="rounded bg-surface-2 px-3 py-1.5 text-[11px] text-muted-foreground transition-colors hover:bg-surface-3 hover:text-foreground"
            >
              {label}
            </button>
          ))}
