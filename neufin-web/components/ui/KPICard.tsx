...
export function KPICard({
    ...
    return (
      ...
      >
        ...
        <div className="mb-3 flex animate-pulse items-center justify-between">
          ...
          <div
            className={`rounded bg-muted-foreground/15 ${compact ? "h-7 w-7" : "h-8 w-8"}`}
          />
        </div>
  ...
  const isConfidence =
    changeLabel === "confidence" && typeof change === "number";
  const showDirection =
    change !== undefined && !isConfidence && Math.abs(change) > 0;
    ...
    >
      ...
      {isConfidence ? (
        ...
        <div className="mt-1.5 flex items-center gap-1">
          ...
          <span
            className={`text-sm font-medium tabular-nums ${change > 0 ? "text-positive" : "text-risk"}`}
          >
            {Math.abs(change)}
          </span>
          {changeLabel ? (
