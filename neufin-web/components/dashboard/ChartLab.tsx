...
export default function ChartLab({ positions, swarmResult }: ChartLabProps) {
  ...
  const markers = useMemo<ChartMarker[]>(() => {
    ...
    if (
      alphaScout?.opportunities?.some((o: any) => o?.symbol === selectedTicker)
    ) {
      out.push({
        time: lastTime,
        position: "belowBar",
    ...
    if (
      taxReport?.liability_top3?.some((t: any) => t?.symbol === selectedTicker)
    ) {
      out.push({
        time: lastTime,
        position: "inBar",
  ...
  const quantSignals = useMemo<QuantOverlaySignal[]>(() => {
    if (!data.length || !swarmResult) return [];
    ...
    const prev = data[Math.max(0, data.length - 6)];
    const nowTime = last?.time;
    const prevTime = prev?.time;
    if (!nowTime || !prevTime) return [];

    const alphaScore = Number(qm?.alpha_score);
    if (Number.isFinite(alphaScore)) {
      out.push({
        time: nowTime,
        type: alphaScore >= 60 ? "buy_zone" : "sell_zone",
        strength: alphaScore,
        label:
          alphaScore >= 60
            ? `Quant buy zone ${Math.round(alphaScore)}`
            : `Quant sell zone ${Math.round(alphaScore)}`,
      });
    }
  ...
  const currentStats = useMemo(() => {
    ...
    const first = data[0];
    const last = data[data.length - 1];
    const dayChangePct = last.open
      ? ((last.close - last.open) / last.open) * 100
      : 0;
    const betaMap = ((swarmResult as any)?.quant_analysis?.beta_map ??
  ...
  const loadTicker = useCallback(async (ticker: string, tf: Timeframe) => {
    ...
    try {
      ...
      const res = await apiGet<
        { data?: CandleData[] } & Record<string, unknown>
      >(
          ...
          {loading ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-3 text-sm text-muted-foreground">
              ...
              <button
                type="button"
                onClick={() =>
                  selectedTicker && void loadTicker(selectedTicker, timeframe)
                }
            ...
            <CandlestickChart
              ...
              onTimeframeChange={(tf) => {
                if (!TIMEFRAMES.includes(tf as Timeframe)) return;
                const next = tf as Timeframe;
                setTimeframe(next);
                if (selectedTicker) void loadTicker(selectedTicker, next);
              }}
              showControls
            />
          )}
...
function Metric({ title, value }: { title: string; value: string }) {
  return (
    <div className="rounded-lg border border-border/60 bg-surface-2 p-3">
      <p className="text-sm font-mono uppercase tracking-widest text-muted-foreground/70">
        {title}
      </p>
