...
export function TrialStatusBanner() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const fresh = await apiGet<SubscriptionStatus>(
  ...
  const banner: BannerState = useMemo(() => {
    ...
    const status =
      subscription.status ??
      (daysRemaining !== null && daysRemaining <= 0 ? "expired" : "trial");
    ...
    return (
      <div className="mb-4 flex max-h-10 min-h-10 items-center justify-between gap-3 border-b border-red-200 bg-red-50 px-4 text-sm text-red-800">
        ...
        <Link
          ...
          className="shrink-0 font-semibold text-red-900 underline underline-offset-2 hover:text-red-950"
        >
          Upgrade
        </Link>
      </div>
