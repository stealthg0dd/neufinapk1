...
export function SwarmBriefingPreview({ swarmReport }: Props) {
  ...
  const briefing = swarmReport.briefing ?? swarmReport.recommendation_summary ?? ''
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{ color: '#1EB8CC', fontSize: 10, fontWeight: 700, letterSpacing: '0.09em', marginBottom: 4 }}>
            SWARM IC BRIEFING
          </div>
        ...
        {date && (
          <div style={{ color: '#64748B', fontSize: 10, whiteSpace: 'nowrap', marginLeft: 12 }}>
      ...
      {briefing && (
        <div style={{ color: '#94A3B8', fontSize: 12, lineHeight: 1.6, marginBottom: 12 }}>
          {briefing.slice(0, 280)}{briefing.length > 280 ? '…' : ''}
        </div>
      )}

      {topRisks.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ color: '#EF4444', fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', marginBottom: 6 }}>
            TOP RISKS
          </div>
          {topRisks.map((risk, i) => (
            <div key={i} style={{
              color: '#F0F4FF', fontSize: 11, paddingLeft: 10,
              borderLeft: '2px solid #EF4444', marginBottom: 5,
            }}>
