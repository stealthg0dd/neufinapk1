...
export default function DashboardClient() {
  ...
  const { loading: authLoading, token, user, signOut, getAccessToken } = useAuth()
  ...
  const [sortKey, setSortKey] = useState<'symbol' | 'weight' | 'price' | 'value'>('value')
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('desc')

  useEffect(() => {
    if (!authLoading && user && user.user_metadata && !user.user_metadata.onboarding_complete) {
    ...
    <div className="min-h-screen bg-[var(--canvas)] flex flex-col md:flex-row pb-section md:pb-0">
        ...
        {!portfolios?.length ? (
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div>
                <label className="text-xs text-[var(--text-muted)] block mb-1">Portfolio</label>
              ...
              <div className="text-right">
                ...
                <div className="mt-2 flex items-center justify-end gap-2 text-sm font-mono flex-wrap">
                  {dayChangePct != null ? (
                    <>
                      <TickerNumber value={dayChangePct} format="percent" showArrow />
                      <span className="text-[var(--text-muted)] text-xs">30d Δ</span>
                    </>
                  ) : (
                    <span className="text-[var(--text-muted)]">—</span>
                  )}
                  <span className="text-[var(--text-muted)] text-xs">
                    As of {new Date().toLocaleTimeString('en-SG', { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
              {[
                ...
                { label: 'Sharpe (est.)', v: sharpe != null ? sharpe.toFixed(2) : '—' },
                ...
                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={history} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                      ...
                      <XAxis
                        dataKey="time"
                        tick={{ fontSize: 10, fill: '#4A5568', fontFamily: 'var(--font-jetbrains)' }}
                  ...
                  <tbody>
                    <AnimatePresence initial={false}>
                      {(sortedPositions ?? []).map((p) => (
                        ...
                        >
                          <td className="p-3 font-mono font-semibold text-[var(--text-primary)]">{p.symbol}</td>
                          <td className="p-3 text-[var(--text-secondary)] text-xs">{p.symbol}</td>
                          ...
                          <td className="p-3 font-mono text-[var(--text-muted)]">—</td>
                          <td className="p-3 font-mono">
                            {metrics?.pnl_pct != null ? (
                              <TickerNumber value={metrics.pnl_pct} format="percent" highlightSign />
                            ) : (
                              '—'
                            )}
                          </td>
      ...
      <aside className="hidden lg:block w-[300px] shrink-0 border-l border-[var(--border)] p-4 bg-[var(--surface-1)]/40">
        ...
        {!notes.length && !loadNotes && (
          <p className="text-xs text-[var(--text-muted)] mb-4">No research notes yet.</p>
        )}
        <div className="space-y-2">
          ...
          {aiReply && (
            <GlassCard className="p-3 mt-2">
              <p className="text-xs text-[var(--text-secondary)] leading-relaxed whitespace-pre-wrap">{aiReply}</p>
            </GlassCard>
          )}
        </div>
      </aside>
