...
function loadPortfolioFromStorage(): {
  ...
  try {
    ...
    const positions: SwarmPosition[] = (parsed.positions ?? []).map(
      (p: any) => ({
        symbol: p.symbol,
        shares: p.shares ?? 0,
        price: p.price ?? 0,
    ...
    const totalValue: number =
      parsed.total_value ?? positions.reduce((s, p) => s + p.value, 0);
...
function cpiSparkline(
  ...
  const seasonal = [
    0.15, -0.05, 0.2, -0.1, 0.1, -0.15, 0.05, 0.1, -0.1, 0.15, -0.1, -0.25,
  ];
  ...
  return months.map((m, i) => ({
    ...
    v:
      Math.round(
        (val + startOffset - (startOffset * i) / 11 + (seasonal.at(i) ?? 0)) *
          10,
      ) / 10,
  }));
}
...
function corrCellStyle(
  v: number,
  isDiag: boolean,
): { bg: string; text: string } {
  if (isDiag) return { bg: "#f1f5f9", text: "#64748b" };
  if (v >= 0.85) return { bg: "rgba(239,68,68,0.12)", text: "#b91c1c" };
  if (v >= 0.7) return { bg: "rgba(245,166,35,0.15)", text: "#b45309" };
  if (v >= 0.5) return { bg: "rgba(30,184,204,0.12)", text: "#0e7490" };
...
function IntelCard({
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-lg border border-border bg-white shadow-sm transition-all duration-300 hover:shadow-md">
      ...
      <div className="flex-1 space-y-2.5 px-3 py-3 text-sm text-slate2">
        {children}
      </div>
    </div>
  );
    ...
    >
      ...
      <div>
        <div className="mb-1 flex justify-between text-xs">
          <span className="uppercase tracking-wide text-muted2">
            CPI YoY (12m)
          </span>
      ...
      <div className="flex gap-3">
        <div className="flex-1 rounded border border-border-light bg-surface-2 px-2 py-1.5">
          <p className="mb-0.5 text-xs uppercase tracking-wider text-muted2">
            Confidence
          </p>
          <p className="font-bold" style={{ color: meta.color }}>
            {conf}%
          </p>
        </div>
        {typeof data.cpi_trend_3m === "number" && (
          <div className="flex-1 rounded border border-border-light bg-surface-2 px-2 py-1.5">
            ...
            <p className="font-bold text-slate2">
              {data.cpi_trend_3m.toFixed(1)}%
            </p>
          </div>
        )}
      </div>
        ...
        <div className="space-y-1 border-t border-border-light pt-2">
          {(data.drivers as string[]).slice(0, 3).map((d, i) => (
            <div key={i} className="flex gap-1.5 items-start">
              <span style={{ color: meta.color }} className="shrink-0 mt-0.5">
                ›
              </span>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `${sentimentCfg.color}15`,
      ...
      {Array.isArray(data.key_drivers) && data.key_drivers.length > 0 && (
        <div className="space-y-1 border-t border-border-light pt-2">
          <p className="mb-1.5 text-xs uppercase tracking-wide text-muted2">
            Key drivers
          </p>
      ...
      {data.positioning_advice && (
        <div className="border-t border-border-light pt-2">
          ...
          <p className="italic text-muted2">
            {data.positioning_advice as string}
          </p>
        </div>
      )}
    </IntelCard>
...
function QuantAnalysisCard({ data }: { data: Record<string, any> }) {
  ...
  const sharpeColor =
    sharpe < 0 ? "#ef4444" : sharpe < 0.5 ? "#b45309" : "#15803d";
  const corrColor =
    avgCorr > 0.8 ? "#ef4444" : avgCorr > 0.65 ? "#b45309" : "#15803d";

  const cmData = data.corr_matrix_data as
    | { symbols: string[]; values: number[][] }
    | undefined;
  const hasCm =
    cmData && Array.isArray(cmData.symbols) && cmData.symbols.length > 0;
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <div>
          <div className="mb-0.5 flex justify-between">
            <span className="text-xs uppercase tracking-wider text-muted2">
              HHI concentration
            </span>
        ...
        <div className="grid grid-cols-3 gap-2 pt-0.5">
          {[
            ...
            <div
              key={label}
              className="rounded border border-border-light bg-surface-2 px-2 py-1.5 text-center"
            >
              <p className="mb-0.5 text-xs uppercase tracking-wider text-muted2">
                {label}
              </p>
            ...
            {Object.entries(data.beta_map as Record<string, number>)
              ...
              .map(([sym, b]) => (
                <div key={sym} className="flex items-center gap-2">
                  <span className="w-12 shrink-0 font-mono text-xs font-semibold text-navy">
                    {sym}
                  </span>
                  <div className="h-[2px] flex-1 overflow-hidden rounded-full bg-surface-3">
                    <div
                      ...
                      style={{
                        ...
                        background:
                          b > 1.8 ? "#ef4444" : b > 1.3 ? "#b45309" : "#15803d",
                        transition: "width 0.7s",
                      }}
                    />
                  </div>
      ...
      {hasCm && (
        <div className="border-t border-border-light pt-2">
          <p className="mb-2 text-xs uppercase tracking-wide text-muted2">
          ...
          <div
            ...
            style={{
              gridTemplateColumns: `40px repeat(${cmData.symbols.length}, 1fr)`,
            }}
          >
            <div />
            {cmData.symbols.map((s) => (
              <div
                key={s}
                className="pb-0.5 text-center font-mono text-sm text-muted2"
              >
                {s}
              </div>
            ))}
        ...
        <div className="space-y-2">
          <div className="space-y-2 rounded border border-border bg-surface-2 p-3 text-center">
            ...
            <div className="font-mono text-xs text-muted2">
              Add{" "}
              <span className="font-semibold text-emerald-700">cost_basis</span>{" "}
              column to CSV to unlock
            </div>
          </div>
          {/* Tax score bar */}
          <div>
...
function AlphaScoutCard({ data }: { data: Record<string, any> }) {
  ...
  return (
    <IntelCard title="Alpha Scout" icon={<Zap size={12} />} accent="#F5A623">
      <p className="pb-1 text-xs uppercase tracking-wide text-muted2">
        Consider diversifying into:
      </p>
      {opps.length === 0 ? (
        <p className="py-4 text-center text-sm text-muted2">
        ...
        <div className="space-y-3">
          {opps.slice(0, 3).map((o: any, i: number) => {
            ...
            const confColor =
              conf >= 75 ? "#15803d" : conf >= 60 ? "#b45309" : "#64748b";
            return (
              <div
                key={i}
                className="space-y-1.5 rounded border border-border-light bg-surface-2/50 p-2"
              >
                {/* Symbol + confidence */}
      ...
      {lines.map((line, idx) => {
        ...
        if (/^### /.test(line)) {
          return (
            <div
              key={idx}
              className="mt-2 text-xs font-bold uppercase tracking-wide text-muted2"
            >
              {line.replace(/^###\s*/, "")}
            </div>
          );
        ...
        if (/^\d+\.\s/.test(line)) {
          ...
          return (
            <div key={idx} className="flex items-start gap-2 pl-2">
              <span className="w-4 shrink-0 font-bold text-primary-dark">
                {num}.
              </span>
        ...
        return <p key={idx}>{renderBoldInline(line, `p${idx}`)}</p>;
      })}
    </div>
  );
}
...
export default function SwarmPage() {
  ...
  const [pollIntervalRef, setPollIntervalRef] = useState<ReturnType<
    typeof setInterval
  > | null>(null);
  ...
  const thesis = (result?.investment_thesis ?? result) as Record<
    string,
    any
  > | null;
  ...
  async function pollStatus(id: string) {
    try {
      ...
      setAgentTrace(
        Array.isArray(status.agent_trace) ? status.agent_trace : [],
      );
  ...
  const startSwarm = async () => {
    ...
    try {
      const data = await apiPost<{ job_id: string; status: JobStatus }>(
        "/api/swarm/analyze",
        {
          positions: positions.map((p) => ({
            symbol: p.symbol,
    ...
    <div className="min-h-screen min-w-0 overflow-x-hidden bg-app font-sans text-navy">
      ...
      <nav className="sticky top-0 z-10 flex min-h-12 flex-wrap items-center justify-between gap-2 gap-y-2 border-b border-border bg-white/95 px-4 py-2 backdrop-blur-sm sm:px-6">
        <div className="flex min-w-0 flex-shrink-0 items-center gap-3">
          <a
            href="/dashboard"
            className="text-sm font-bold tracking-wide text-primary transition-colors hover:text-primary-dark"
          >
            NEUFIN
          </a>
        ...
        <div className="flex min-w-0 flex-1 flex-wrap items-center justify-end gap-2">
          ...
          <button
            ...
            title={
              positions.length === 0
                ? "Upload a portfolio on neufin.app first"
                : undefined
            }
      ...
      <div className="max-w-[1600px] mx-auto px-4 py-5 grid grid-cols-1 xl:grid-cols-12 gap-4">
          ...
          <div className="flex h-full min-h-[420px] flex-col overflow-hidden rounded-lg border border-border bg-white shadow-sm">
            <div className="flex shrink-0 items-center justify-between border-b border-border-light bg-surface-2 px-4 py-2">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-medium uppercase text-muted2">
              ...
              {!thesis && !isRunning && (
                <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
                  ...
                  <p className="text-sm text-slate2">
                    Run IC analysis to generate the Investment Committee report.
                  </p>
                </div>
              )}
              {isRunning && !thesis && (
            ...
            {thesis && (
              <div className="flex shrink-0 flex-wrap gap-3 border-t border-border-light bg-surface-2 px-4 py-2">
                <MetaItem
                  label="REGIME"
                  value={thesis.regime ?? "N/A"}
                  color="blue"
                />
                <MetaItem
                  label="β"
                  value={thesis.weighted_beta?.toFixed(2) ?? "—"}
                  color="amber"
                />
                <MetaItem
                  label="SHARPE"
                  ...
                  color={
                    (thesis.sharpe_ratio ?? 0) > 1
                      ? "green"
                      : (thesis.sharpe_ratio ?? 0) > 0
                        ? "amber"
                        : "red"
                  }
                />
                <MetaItem
                  label="ρ avg"
                  value={thesis.avg_correlation?.toFixed(3) ?? "—"}
                  color="amber"
                />
              </div>
            )}
        ...
        <div className="space-y-3 xl:col-span-2">
          ...
          <div className="overflow-hidden rounded-lg border border-border bg-white shadow-sm">
            ...
            <div className="space-y-2 px-3 py-2">
              {positions.length === 0 ? (
                <div className="py-2 text-center text-sm text-muted2">
          ...
          {thesis && Object.keys(sb).length > 0 && (
            <div className="overflow-hidden rounded-lg border border-border bg-white shadow-sm">
              ...
              <div className="space-y-2.5 px-3 py-3">
                <ScorePill
                  label="HHI"
                  value={sb.hhi_concentration ?? 0}
                  max={25}
                  color="#F5A623"
                />
                <ScorePill
                  label="Beta"
                  value={sb.beta_risk ?? 0}
                  max={25}
                  color="#64748b"
                />
                <ScorePill
                  label="Tax α"
                  value={sb.tax_alpha ?? 0}
                  max={20}
                  color="#16a34a"
                />
                <ScorePill
                  label="Corr"
                  value={sb.correlation ?? 0}
                  max={30}
                  color="#1EB8CC"
                />
          ...
          <div className="space-y-1 py-2 text-center">
            <div className="text-xs text-muted2">
              Press{" "}
              <kbd className="rounded border border-border bg-surface-2 px-1 py-0.5 font-mono text-muted2">
                ⌘K
              </kbd>{" "}
              to query agents
            </div>
            <div className="text-sm uppercase tracking-wide text-muted2">
        ...
        {/* Risk Matrix */}
        {thesis &&
          ((thesis as any).stress_results?.length > 0 ||
            (thesis as any).risk_factors?.length > 0) &&
          (() => {
            const clusters = ((thesis as any).risk_factors ?? []).map(
              (f: any) => ({
                ticker: f.symbol,
                beta: f.beta ?? 1.0,
            ...
            const stressResults = ((thesis as any).stress_results ?? []).map(
              (s: any) => ({
            ...
            <div className="flex items-center gap-3">
              <svg viewBox="0 0 120 120" className="h-16 w-16">
                ...
                <text
                  ...
                  y="65"
                  textAnchor="middle"
                  className="fill-navy text-xl font-bold"
                >
      ...
      {jobStatus === "complete" && result ? (
        <SwarmSourcesPanel
          sources={
            (result as Record<string, unknown>).sources as
              | SwarmSourcesPayload
              | undefined
          }
          observability={
            (result as Record<string, unknown>).observability as
              | SwarmObservabilityPayload
              | undefined
          }
        />
      ) : null}
          ...
          >
            {[
              thesis.market_regime && (
                <MarketRegimeCard key="regime" data={thesis.market_regime} />
              ),
              thesis.strategist_intel && (
                <StrategistIntelCard
                  key="strategist"
                  data={thesis.strategist_intel}
                />
              ),
              thesis.quant_analysis && (
                <QuantAnalysisCard key="quant" data={thesis.quant_analysis} />
              ),
              thesis.tax_report && (
                <TaxOptimizationCard key="tax" data={thesis.tax_report} />
              ),
              thesis.risk_sentinel && (
                <RiskWatchdogCard key="risk" data={thesis.risk_sentinel} />
              ),
              thesis.alpha_scout && (
                <AlphaScoutCard key="alpha" data={thesis.alpha_scout} />
              ),
            ]
              .filter(Boolean)
              .map((card, i) => (
...
const META_TIPS: Record<string, string> = {
  REGIME:
    "Macro regime detected by FRED CPI analysis — growth, inflation, stagflation, recession, or risk-off",
  β: "Weighted portfolio beta vs S&P 500. >1 = amplified market moves, <1 = defensive",
  SHARPE:
    "Risk-adjusted return ratio. >1 is good, >2 is excellent, <0 is losing on a risk-adjusted basis",
  "ρ avg":
    "Average pairwise Pearson correlation between holdings. High correlation = limited diversification",
};
...
function MetaItem({
  ...
  const inner = (
    <div className="flex cursor-help items-center gap-1 text-xs">
      <span className="border-b border-dotted border-border uppercase text-muted2">
        {label}:
      </span>
