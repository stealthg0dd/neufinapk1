...
export default function ShareCard({ data }: { data: DNAShare }) {
  ...
  const scoreColor = data.dna_score >= 70 ? '#22c55e' : data.dna_score >= 40 ? '#f59e0b' : '#ef4444'
  ...
  return (
    ...
    >
      ...
      {/* ── Share actions ──────────────────────────────────────────── */}
      <motion.div variants={fadeUp} className="w-full grid grid-cols-2 sm:grid-cols-4 gap-2">
      ...
      <motion.div variants={fadeUp} className="w-full pb-6">
        <div className="border-t border-shell-border/60 pt-6 grid sm:grid-cols-3 gap-4 text-center">
          {[
            ...
            { icon: '🔒', title: 'Private by Default', body: 'Your position details are never made public. Only insights are shared.' },
          ].map(({ icon, title, body }) => (
