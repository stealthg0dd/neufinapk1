...
export default function AdminUserDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setErr(null)
    try {
      const res = await apiFetch(`/api/admin/users/${encodeURIComponent(id)}`, { cache: 'no-store' })
    ...
    <div className="p-6 max-w-xl space-y-4">
      ...
      <div className="flex flex-col gap-2">
        ...
        <button
          ...
          onClick={() =>
            doAction('Reset password link', async () => {
              ...
              if (!res.ok) throw new Error((j as { message?: string }).message || `${res.status}`)
        ...
        <button
          ...
          onClick={() => {
            if (!confirm('Permanently delete this user from Auth + database?')) return
