    ...
    <div className="p-6 max-w-6xl space-y-4">
      <div className="flex flex-wrap justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">All issued keys (hashed at rest).</p>
        </div>
        <button
          type="button"
          onClick={async () => {
            ...
            if (!pid) return
            try {
              const out = await apiPost<{ key?: string }>('/api/admin/api-keys/issue', {
                partner_id: pid,
                name: 'admin-issued',
              })
          ...
          <tbody className="divide-y divide-zinc-800">
            {keys.map((k) => (
              <tr key={k.id} className="hover:bg-zinc-900/40">
                <td className="px-3 py-2 text-zinc-300">{k.partner_email || k.partner_id}</td>
                <td className="px-3 py-2 font-mono text-xs text-zinc-400">{k.key_masked}</td>
                <td className="px-3 py-2 text-zinc-500">{k.created_at?.slice(0, 10) || '—'}</td>
                <td className="px-3 py-2 text-zinc-500">{k.last_used_at?.slice(0, 10) || '—'}</td>
                ...
                <td className="px-3 py-2 text-right space-x-2">
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        await apiPost(`/api/admin/api-keys/${encodeURIComponent(k.id)}/revoke`, {})
                        await load()
                      } catch (e) {
                        alert(e instanceof Error ? e.message : String(e))
                      }
                  ...
                  <button
                    ...
                    onClick={async () => {
                      const v = window.prompt('New daily rate limit', String(k.rate_limit_daily ?? 10000))
                      if (!v) return
