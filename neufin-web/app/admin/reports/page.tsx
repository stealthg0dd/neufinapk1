...
export default function AdminReportsPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await apiFetch('/api/admin/reports?limit=100', { cache: 'no-store' })
  ...
  return (
    <div className="p-6 max-w-5xl space-y-4">
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-1">Recent advisor + swarm report rows.</p>
      </div>
          ...
          <tbody className="divide-y divide-zinc-800">
            {rows.map((r) => (
              <tr key={`${r.source}-${r.id}`} className="hover:bg-zinc-900/40">
                <td className="px-3 py-2 text-zinc-400">{r.created_at || '—'}</td>
                <td className="px-3 py-2">{r.source}</td>
                <td className="px-3 py-2 font-mono text-xs">{r.id}</td>
                <td className="px-3 py-2 text-xs text-zinc-500">
                  {r.advisor_id || r.user_id || '—'}
                </td>
                <td className="px-3 py-2">{r.is_paid == null ? '—' : r.is_paid ? 'yes' : 'no'}</td>
              </tr>
            ))}
          </tbody>
        </table>
