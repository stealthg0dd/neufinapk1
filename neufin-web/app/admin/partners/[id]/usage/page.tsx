...
type UsagePayload = {
  ...
  days: number
  daily_totals: Daily[]
  keys: { id: string; key_masked: string; name?: string; is_active?: boolean; daily: Daily[] }[]
}
          ...
          <div className="rounded-xl border border-zinc-800 overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-zinc-800">
                {data.keys.map((k) => (
                  <tr key={k.id}>
                    <td className="px-3 py-2 font-mono text-xs">{k.key_masked}</td>
                    <td className="px-3 py-2 text-zinc-400">{k.name || '—'}</td>
                    <td className="px-3 py-2">{k.is_active ? 'yes' : 'no'}</td>
                    <td className="px-3 py-2 text-right tabular-nums">
                      {k.daily.reduce((s, d) => s + d.calls, 0).toLocaleString()}
                    </td>
                  </tr>
                ))}
              </tbody>
