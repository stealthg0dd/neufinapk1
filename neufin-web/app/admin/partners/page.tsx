  ...
  const c =
    h === "GREEN"
      ...
      : h === "AMBER"
        ...
        : "bg-red-400";
  return (
    <span className={`inline-block h-2 w-2 rounded-full ${c}`} title={h} />
  );
}
    ...
    <div className="p-6 max-w-6xl space-y-4">
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-1">
          B2B accounts with API keys (usage last 30 days).
        </p>
      </div>
            ...
            {rows.map((p) => (
              <tr key={p.id} className="hover:bg-zinc-900/40">
                ...
                <td className="px-3 py-2 tabular-nums">
                  {p.api_calls_30d.toLocaleString()}
                </td>
                <td className="px-3 py-2">
                  {p.mrr_usd != null ? `$${p.mrr_usd}` : "—"}
                </td>
                <td className="px-3 py-2">{p.status}</td>
                <td className="px-3 py-2 flex items-center gap-2">
                  {healthDot(p.integration_health)}
                  <span className="text-xs text-zinc-500">
                    {p.integration_health}
                  </span>
                </td>
                <td className="px-3 py-2 text-right space-x-2 whitespace-nowrap">
                  <Link
                    href={`/admin/partners/${p.id}/usage`}
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (!confirm("Rotate API key? Old keys stop working."))
                        return;
                      try {
                        ...
                        if (out.key)
                          window.prompt("New key (copy now)", out.key);
                      } catch (e) {
