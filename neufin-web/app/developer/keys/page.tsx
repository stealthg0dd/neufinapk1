...
export default function DeveloperKeysPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 403) {
        setError("Enterprise plan required for API access.");
        return;
      }
  ...
  async function handleRevoke(id: string) {
    if (!token || !confirm("Revoke this API key? This cannot be undone."))
      return;
    setRevoking(id);
    try {
      ...
      <div className="mx-auto max-w-3xl space-y-8">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted2">
              Manage your NeuFin Enterprise API keys
            </p>
          </div>
        </div>
        ...
        {newKey && (
          <div className="space-y-3 rounded-xl border border-success2/30 bg-success2/5 p-5">
            ...
            <button
              ...
              className="text-xs text-muted2 hover:text-navy"
            >
              Dismiss
            </button>
          </div>
        ...
        <div className="space-y-4 rounded-2xl border border-border bg-white p-6 shadow-sm">
          ...
          <p className="text-sm text-muted2">
            Rate limit: 10,000 requests/day per key. Keys are stored as hashes —
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-white shadow-sm">
          ...
          {loading ? (
            ...
            <div className="p-10 text-center text-sm text-muted2">
            ...
            <div className="divide-y divide-border-light bg-white">
              {mergedKeys.map((k) => (
                <div
                  key={k.id}
                  className="flex items-center justify-between px-5 py-4 transition-colors hover:bg-surface-2/60"
                >
                  <div className="min-w-0 space-y-1">
                    ...
                    <p className="text-sm text-muted2">
                      ...
                      {fmt(k.last_used_at)} · Usage this month{" "}
                      {k.usage_this_month?.toLocaleString() || 0} calls · Rate
                      limit {k.rate_limit_per_day.toLocaleString()} calls/day
                    </p>
                    {k.key_prefix && (
                      <code className="font-mono text-xs text-slate2">
                        {k.key_prefix}…
                      </code>
                    )}
                  </div>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-navy/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl border border-border bg-white p-6 shadow-lg">
            ...
            <p className="mt-1 text-sm text-muted2">
              Example: Production — MYTHEO Integration
            </p>
            ...
            <p className="mt-3 text-xs text-warning2">
