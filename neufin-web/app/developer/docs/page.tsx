...
const ENDPOINTS: Endpoint[] = [
  {
    method: "GET",
    path: "/api/research/regime",
    summary:
      "Get current market regime (risk-on, risk-off, stagflation, recovery, recession_risk)",
    auth: "api_key",
    plan: "enterprise",
    response:
      '{ "regime": "risk_on", "confidence": 0.82, "started_at": "2026-03-15T00:00:00Z", "supporting_signals": {...} }',
  },
  {
    method: "GET",
    ...
    plan: "enterprise",
    response:
      '{ "notes": [ { "id": "...", "title": "...", "executive_summary": "...", "generated_at": "..." } ] }',
  },
  {
    method: "POST",
    path: "/api/research/query",
    summary:
      "Semantic search across research knowledge base using natural language",
    auth: "api_key",
    plan: "enterprise",
    request:
      '{ "query": "impact of Fed rate cuts on Singapore REITs", "limit": 5 }',
    response:
      '{ "results": [ { "type": "note|signal|event", "title": "...", "similarity": 0.91, "summary": "..." } ] }',
  },
  {
    method: "GET",
    path: "/api/research/signals",
    summary:
      "Latest macro signals (interest rates, inflation, GDP) with significance scores",
    auth: "api_key",
    plan: "enterprise",
    response:
      '{ "signals": [ { "signal_type": "interest_rate", "region": "US", "value": 5.25, "significance": "high" } ] }',
  },
  {
    method: "POST",
    path: "/api/analyze-dna",
    ...
    response:
      '{ "dna_score": 78, "investor_type": "Balanced Trader", "bias_scores": { "disposition_effect": 0.72 }, "market_context": { "regime": "risk_on" } }',
  },
  {
    method: "GET",
    path: "/api/plans",
...
export default function DeveloperDocsPage() {
  ...
  const [endpoint, setEndpoint] = useState<"swarm" | "regime" | "chart">(
    "regime",
  );
  ...
  const [response, setResponse] = useState<unknown>({
    message: "Send a request to start.",
  });
          ...
          pollRef.current = setInterval(async () => {
            try {
              const pollRes = await fetch(
                `${BASE_URL}/api/swarm/status/${jobId}`,
                {
                  headers: {
                    Authorization: `Bearer ${apiKey}`,
                    "X-NeuFin-API-Key": apiKey,
    ...
    <div className="min-h-screen bg-app text-navy">
      ...
      <div className="mx-auto max-w-6xl space-y-6 px-6 py-section">
        ...
        <section className="rounded-xl border border-border bg-white p-4 shadow-sm">
          ...
          <p className="text-sm text-slate2">
            Getting started: create account → Developer → copy API key.
          </p>
        ...
        <section className="rounded-xl border border-border bg-white p-4 shadow-sm">
          <h2 className="text-lg font-semibold text-navy">
            2) POST /api/swarm/analyze
          </h2>
          <p className="text-sm text-slate2">
            Submit a portfolio for 7-agent IC analysis. Returns immediately with
            job_id. Poll /status/{"{job_id}"} for progress.
          </p>
  ...
  "estimated_seconds": 75
        ...
        <section className="rounded-xl border border-border bg-white p-4 shadow-sm">
          <h2 className="text-lg font-semibold text-navy">
            3) GET /api/swarm/status/{"{job_id}"}
          </h2>
          <p className="text-sm text-slate2">
            Poll every 3s to track agent progress and get results.
          </p>
          <pre className="mt-2 overflow-x-auto rounded-lg border border-border bg-surface-2 p-3 font-mono text-sm text-slate2">{`{
  "job_id": "8f2a4b1c-...",
  "status": "running",
  ...
  "agent_trace": [
    ...
    {"agent": "sector_agent", "status": "running", "summary": "..."}
        ...
        <section className="rounded-xl border border-border bg-white p-4 shadow-sm">
          <h2 className="text-lg font-semibold text-navy">
            4) GET /api/swarm/result/{"{job_id}"}
          </h2>
          <p className="text-sm text-slate2">
            Fetch full IC briefing when status == complete.
          </p>
          <pre className="mt-2 overflow-x-auto rounded-lg border border-border bg-surface-2 p-3 font-mono text-sm text-slate2">{`{
  "ic_briefing": {
    "portfolio_analyst": {...},
  ...
  "updated_at": "2026-04-10T08:00:00Z"
        ...
        <section className="rounded-xl border border-border bg-white p-4 shadow-sm">
          <h2 className="text-lg font-semibold text-navy">
            6) GET /api/portfolio/chart/{"{ticker}"}
          </h2>
          <p className="text-sm text-slate2">
            OHLCV chart data for candlestick rendering.
          </p>
          <p className="text-sm text-slate2">
            Params: period = 1mo | 3mo | 6mo | 1y | 3y
          </p>
          <pre className="mt-2 overflow-x-auto rounded-lg border border-border bg-surface-2 p-3 font-mono text-sm text-slate2">
            [
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div>
              <select
                value={endpoint}
                onChange={(e) =>
                  setEndpoint(e.target.value as "swarm" | "regime" | "chart")
                }
              ...
              >
                ...
                <option value="chart">
                  GET /api/portfolio/chart/{"{ticker}"}
                </option>
              </select>
              <textarea
                value={body}
