...
function LeadModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4" onClick={onClose}>
      <div
        className="w-full max-w-lg rounded-2xl border border-gray-700 bg-gray-900 p-6 space-y-5 shadow-2xl"
      ...
      >
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400">{lead.company ?? "—"} · {lead.role ?? "—"}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <p className={`font-medium ${AUM_COLORS[lead.aum_range ?? ""] ?? "text-gray-300"}`}>{lead.aum_range ?? "—"}</p>
          </div>
          <div>
          ...
          <div>
            <p className="text-xs text-gray-500 mb-0.5">Interested Plan</p>
            <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${PLAN_BADGE[lead.interested_plan ?? ""] ?? "bg-gray-700 text-gray-300"}`}>
              {lead.interested_plan ?? "—"}
            </span>
          </div>
          <div>
            <p className="text-xs text-gray-500 mb-0.5">Created</p>
            <p className="text-gray-300">{fmt(lead.created_at)}</p>
          </div>
          {lead.contacted_at && (
            <div>
              <p className="text-xs text-gray-500 mb-0.5">Contacted</p>
              <p className="text-gray-300">{fmt(lead.contacted_at)}</p>
            </div>
          )}
        </div>
...
export default function LeadsAdminPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-950 p-6 max-w-full space-y-6">
      ...
      <div className="px-4 py-6 space-y-6">
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">B2B sales management</p>
          </div>
        ...
        {stats && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className="rounded-xl border border-gray-800 bg-gray-900 p-4 text-center">
              ...
              <p className="text-xs text-gray-500 mt-0.5">Won This Month</p>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-4 overflow-x-auto pb-6">
          {COLUMNS.map((col) => {
            ...
            return (
              <div
                key={col.key}
                className={`min-w-[240px] max-w-[280px] flex-shrink-0 rounded-xl border-t-2 border border-gray-800 bg-gray-900/60 ${col.color}`}
              >
                {/* Column header */}
                <div className="px-4 pt-4 pb-2 flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-gray-200">{col.label}</h3>
                  <span className="rounded-full bg-gray-800 px-2 py-0.5 text-xs text-gray-400 font-medium">
                    {colLeads.length}
                  </span>
                </div>
                ...
                <div className="px-3 pb-3 space-y-2">
                  {colLeads.length === 0 && (
                    <div className="rounded-lg border border-dashed border-gray-700 p-4 text-center text-xs text-gray-600">
                      No leads
                    </div>
                  )}
                  {colLeads.map((lead) => (
                    <button
                      ...
                      className="w-full text-left rounded-lg border border-gray-700 bg-gray-900 p-3 hover:border-gray-500 hover:bg-gray-800/80 transition-colors space-y-2"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-sm font-medium text-gray-100 leading-tight">{lead.name}</p>
                        {lead.aum_range && (
                          <span className={`text-xs font-medium whitespace-nowrap flex-shrink-0 ${AUM_COLORS[lead.aum_range] ?? "text-gray-500"}`}>
                            {lead.aum_range}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        {lead.interested_plan ? (
                          <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${PLAN_BADGE[lead.interested_plan] ?? "bg-gray-700 text-gray-400"}`}>
                            {lead.interested_plan}
                          </span>
                        ) : <span />}
                        <span className="text-[10px] text-gray-600">{fmt(lead.created_at)}</span>
                      </div>
                      {lead.source && (
                        <p className="text-[10px] text-gray-600">via {lead.source.replace(/_/g, " ")}</p>
                      )}
                    </button>
                  ))}
                </div>
