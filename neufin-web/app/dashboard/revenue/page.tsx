...
function StatCard({
  ...
  return (
    <div
      className={`rounded-xl border bg-white p-5 shadow-sm ${accent ?? "border-[#E2E8F0]"}`}
    >
    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-xs">
        ...
        <div className="flex items-center gap-3">
          ...
          {conversion !== "—" && (
            <span className="text-[#94A3B8]">{conversion} CVR</span>
          )}
        </div>
      </div>
...
export default function RevenuePage() {
  ...
  const month = new Date().toLocaleString("en-SG", {
    month: "long",
    year: "numeric",
  });
    ...
    return (
      <div className="min-h-screen bg-transparent p-6 max-w-7xl mx-auto">
        <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-6">
          ...
          <button
            onClick={load}
            className="mt-4 text-xs text-red-400 border border-red-500/30 rounded-md px-3 py-1.5 hover:bg-red-500/10 transition-colors"
          >
            Retry
          </button>
        </div>
  ...
  const { pct: revDeltaPct, up: revDeltaUp } = revDelta(
    stats.revenue_this_month_usd,
    stats.revenue_last_month_usd,
  );
  const mrr = stats.revenue_this_month_usd;
  const arr = mrr * 12;
  const totalUsers =
    stats.active_subscribers + stats.trial_users + stats.expired_users;
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-8">
        ...
        <div className="section-header">
          <div>
            ...
            <p>{month} · Stripe + Supabase</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Link
              href="/dashboard/admin/leads"
              className="rounded-lg border border-[#E2E8F0] px-3 py-1.5 text-xs text-navy/90 transition-colors hover:border-[#94A3B8]"
            >
              Leads →
            </Link>
            <button
            ...
            <div className="data-card rounded-xl">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                ...
                <div className="text-center">
                  ...
                  {leads.last_week > 0 && (
                    <p
                      className={`text-xs mt-0.5 ${leads.this_week >= leads.last_week ? "text-emerald-400" : "text-red-400"}`}
                    >
                      {leads.this_week >= leads.last_week ? "▲" : "▼"} vs{" "}
                      {leads.last_week} last wk
                    </p>
                  )}
                </div>
                <div className="text-center">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(STATUS_LABELS).map(([key, label]) => {
                  ...
                  return (
                    <span
                      key={key}
                      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium ${STATUS_COLORS[key] ?? "bg-[#F8FAFC] text-navy/90"}`}
                    >
                      {label}
              ...
              <tbody className="divide-y divide-[#F1F5F9]">
                ...
                {stats.recent_purchases.map((p, i) => (
                  <tr key={i} className="hover:bg-[#F8FAFC] transition-colors">
                    ...
                    <td className="px-4 py-3 text-navy tabular-nums font-medium">
