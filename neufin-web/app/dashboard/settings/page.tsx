...
function Field({ label, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  return (
    <div className="flex flex-col gap-1">
      ...
      <input
        {...props}
        className="bg-shell border border-shell-border rounded-lg px-3 py-2 text-sm text-shell-fg focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 disabled:opacity-50 disabled:cursor-not-allowed"
      />
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl overflow-hidden border border-[#2A3550]">
        <div className="h-1.5" style={{ background: wlEnabled ? (brandColor || '#1EB8CC') : '#1EB8CC' }} />
        <div className="bg-[#0B0F14] px-4 py-3 flex items-center gap-3">
          {displayLogo ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={displayLogo} alt="Firm logo" style={{ height: 28, objectFit: 'contain' }} />
          ) : (
      ...
      {!editing ? (
        ...
        <div className="flex flex-col gap-4 p-4 bg-[#0D1118] rounded-xl border border-[#2A3550]">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <label htmlFor="wl-settings" className="text-sm text-[#F0F4FF] cursor-pointer">
              Enable white-label branding on reports
            </label>
          </div>

          <div>
            <label className="block text-xs text-[#64748B] mb-1">Firm name</label>
          ...
          <div>
            <label className="block text-xs text-[#64748B] mb-1">Advisor name (appears on reports)</label>
            <input type="text" value={advisorName} onChange={(e) => setAdvisorName(e.target.value)}
              placeholder="Jane Smith, CFA"
              className="w-full bg-[#0B0F14] border border-[#2A3550] rounded-lg px-3 py-2 text-sm text-[#F0F4FF] placeholder-[#3A4560] focus:outline-none focus:ring-2 focus:ring-[#1EB8CC]/40" />
          </div>
          <div>
            <label className="block text-xs text-[#64748B] mb-1">Contact email for reports</label>
          ...
          {/* Logo */}
          <div>
            <label className="block text-xs text-[#64748B] mb-1">Firm logo</label>
            <input ref={fileRef} type="file" accept="image/png,image/svg+xml,image/jpeg,image/webp" onChange={handleLogoSelect} className="hidden" />
            <div onClick={() => fileRef.current?.click()}
              ...
              {displayLogo
                // eslint-disable-next-line @next/next/no-img-element
                ? <img src={displayLogo} alt="Logo" style={{ height: 28, objectFit: 'contain' }} />
                : <div className="text-[#64748B] text-xs">Click to upload (PNG/SVG/JPEG)</div>
              }
          ...
          {/* Brand color */}
          <div>
            <label className="block text-xs text-[#64748B] mb-1">Primary brand color</label>
            <div className="flex items-center gap-3">
