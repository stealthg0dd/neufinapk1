...
function severityClass(sev: ScanFinding["severity"]): string {
  switch (sev) {
    ...
    case "LOW":      return "bg-[#F8FAFC] text-[#64748B]"
  }
}
...
function RepoHealthCard({
  ...
  return (
    <div className={`rounded-xl border p-4 space-y-3 ${
      repo.status === "live"    ? "border-[#E2E8F0] bg-white" :
      ...
      <div className="space-y-1">
        {repo.last_seen && (
          <p className="text-sm text-[#94A3B8]">
            Last seen: <span className="text-[#64748B]">{relativeTime(repo.last_seen)}</span>
          </p>
        )}
        {repo.version && (
          <p className="text-sm text-[#94A3B8]">
            Version: <span className="text-[#64748B] font-mono">{repo.version}</span>
          </p>
        )}
        {deployment?.deployed_at && (
          <p className="text-sm text-[#94A3B8]">
            Deployed: <span className="text-[#64748B]">{relativeTime(deployment.deployed_at)}</span>
            {deployment.commit_sha && (
              <span className="ml-1 font-mono text-[#94A3B8]">{deployment.commit_sha.slice(0, 7)}</span>
            )}
          </p>
        )}
        {deployment?.commit_msg && (
          <p className="text-sm text-[#94A3B8] truncate" title={deployment.commit_msg}>
            {deployment.commit_msg}
          </p>
        )}
      </div>
    ...
    return (
      <div className="rounded-xl border border-[#E2E8F0] bg-white p-5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-navy/90">All clear — no open findings</span>
        </div>
      </div>
    )
  }
...
function providerDotClass(h: ProviderStats | RateLimitStats): string {
  ...
  if ("cooldown_s" in h && h.cooldown_s > 0) return "bg-yellow-400 animate-pulse"
    ...
    <div className="rounded-2xl border border-[#E2E8F0] bg-white p-5">
      ...
      <div className="flex items-baseline justify-between mb-2">
        <div className="flex items-baseline gap-2">
          ...
          <span className="text-sm text-[#94A3B8]">/ ${(budget.daily_cap ?? 15).toFixed(2)}</span>
        </div>
      ...
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-baseline gap-2">
          ...
          <span className="text-xs text-[#94A3B8]">/ ${(budget.monthly_cap ?? 400).toFixed(2)}</span>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between text-sm">
        ...
        <span className="text-[#64748B] tabular-nums">${costToday.toFixed(4)}</span>
      </div>
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
        <div>
          ...
          <p className="text-lg font-bold text-navy leading-none tabular-nums">{agentCount}</p>
        </div>
        <div>
...
function Skeleton({ className = "" }: { className?: string }) {
  return (
    <div
      className={`animate-pulse rounded-lg bg-[#F8FAFC] ${className}`}
    />
  )
}
...
export default function AgentOSDashboard() {
  ...
  const SL = "text-sm font-semibold uppercase tracking-widest text-[#94A3B8] mb-2.5 block"
  ...
  const sortedProviders = Object.entries(providers).sort(([an, av], [bn, bv]) => {
  ...
  const VENTURE_ORDER = ["neufin", "arisole", "neumas", "apex_golf", "defquant", "ctech_corporate"]
  const companies = [
    ...VENTURE_ORDER.filter((c) => c in agents),
    ...Object.keys(agents).filter((c) => !VENTURE_ORDER.includes(c)).sort(),
  ]
          ...
          {!neufinHealth ? (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="rounded-xl border border-[#E2E8F0] bg-white h-28 animate-pulse" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
            ...
            {sortedProviders.length === 0 && (
              <p className="col-span-full text-sm text-[#94A3B8]">No providers registered.</p>
            )}
          </div>
        </section>
