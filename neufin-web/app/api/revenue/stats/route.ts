...
export async function GET(req: NextRequest) {
  ...
  const [
    ...
    safeCountGte("advisor_reports", "created_at", monthISO, { is_paid: "true" }),
  ])
  ...
  for (const p of emailProfiles ?? []) {
    emailMap[(p as {id: string; email: string}).id] = (p as {id: string; email: string}).email ?? ""
  }

  type PurchaseRow = { advisor_id: string; plan_type: string | null; amount_cents: number | null; created_at: string }
  const recentPurchases = (recentPurchasesResult.data ?? []).map((r: PurchaseRow) => ({
