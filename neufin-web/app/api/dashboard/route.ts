...
export async function GET() {
  ...
  const [briefs, routerStatus, callLogsRaw, _agents] = await Promise.all([
    agentGet<{ briefs?: unknown[] } | unknown[]>("/morning-engine/latest", { briefs: [] }),
    agentGet<{
  ...
  const data: DashboardData = {
    ...
    rateLimits: {},
    callLogs:  Array.isArray(callLogsRaw) ? (callLogsRaw as DashboardData["callLogs"]) : [],
    budget: {
      daily_spend:      budget.daily_spend      ?? 0,
      daily_cap:        budget.daily_cap        ?? 15,
      daily_remaining:  budget.daily_remaining  ?? (budget.daily_cap ?? 15) - (budget.daily_spend ?? 0),
      monthly_spend:    budget.monthly_spend    ?? 0,
      monthly_cap:      budget.monthly_cap      ?? 400,
      monthly_remaining: budget.monthly_remaining ?? (budget.monthly_cap ?? 400) - (budget.monthly_spend ?? 0),
    },
  }
