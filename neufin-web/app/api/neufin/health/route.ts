...
export async function GET() {
  if (!BASE || !KEY) {
    ...
    return NextResponse.json({
      ...
      repos: offlineRepos,
      top_findings: [],
      deployments: NEUFIN_REPOS.map((r) => ({
        repo_id: r,
        deployed_at: null,
        commit_sha: null,
        commit_msg: null,
  ...
  const [repoStatuses, scanData, ...deployResults] = await Promise.all([
    safeGet<
      Record<
        string,
        { last_seen?: string; version?: string; environment?: string }
      >
    >("/api/repos/neufin/heartbeats", {}),
    safeGet<{ findings?: ScanFinding[] }>("/api/repos/neufin-backend/scans", {
      findings: [],
    }),
  ...
  // Error rate
  const unresolvedCritical = allFindings.filter(
    (f) => !f.resolved && f.severity === "CRITICAL",
  ).length;
  const unresolvedHigh = allFindings.filter(
