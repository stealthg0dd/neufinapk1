...
function normalizeStripeWebhookSecret(raw: string | undefined): string | undefined {
...
function resolveWebhookSecret(): string | undefined {
  const cli = normalizeStripeWebhookSecret(process.env.STRIPE_WEBHOOK_SECRET_CLI)
  if (cli) return cli
...
function warnIfLocalStripeListenSecretMissing(): void {
  ...
  if (normalizeStripeWebhookSecret(process.env.STRIPE_WEBHOOK_SECRET_CLI)) return
  console.warn(
...
export async function POST(req: Request) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    console.log(
      ...
      process.env.STRIPE_WEBHOOK_SECRET_CLI ? 'STRIPE_WEBHOOK_SECRET_CLI' : 'STRIPE_WEBHOOK_SECRET',
      'prefix:',
  ...
  try {
    ...
    const localHint =
      process.env.NODE_ENV !== 'production'
        ...
        : ''
    return new Response(`Webhook Error: ${message}${localHint}`, { status: 400 })
  }
  ...
  try {
    ...
    await reportStripeWebhookFailure('webhook_handler_unhandled', {
      stripe_event_id: event.id,
      stripe_event_type: event.type,
    }, handlerErr)
  }
...
async function reportStripeWebhookFailure(
  ...
  console.error('[webhook] CRITICAL — subscription data may be out of sync:', payload)
  ...
  try {
    ...
    const err =
      error instanceof Error
        ? error
        : new Error(`[stripe webhook] ${reason}`)
...
async function dispatchStripeWebhookEvent(
  ...
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      if (!email) {
        await reportStripeWebhookFailure('checkout.session.completed_missing_email', {
          stripe_event_id: event.id,
      ...
      if (error) {
        await reportStripeWebhookFailure('checkout.session.completed_supabase_error', {
          stripe_event_id: event.id,
          checkout_session_id: session.id,
          customer_id: customerId ?? null,
      ...
      if (!rows?.length) {
        await reportStripeWebhookFailure('checkout.session.completed_no_profile_row', {
          stripe_event_id: event.id,
          checkout_session_id: session.id,
          customer_id: customerId ?? null,
          email_domain: emailDomainOnly(email),
          hint: 'No user_profiles row matched this checkout email — paid user may need manual tier fix',
        }, new Error('No user_profiles row updated for checkout.session.completed'))
        return
      }

      console.log(`[webhook] Upgraded to advisor: ${email_domain_only_log(email)}`)
      return
    }
    ...
    case 'invoice.paid': {
      ...
      if (error) {
        await reportStripeWebhookFailure('invoice.paid_supabase_error', {
          stripe_event_id: event.id,
          invoice_id: invoice.id,
      ...
      if (!rows?.length) {
        await reportStripeWebhookFailure('invoice.paid_no_profile_row', {
          stripe_event_id: event.id,
          invoice_id: invoice.id,
          customer_id: customerId,
    ...
    case 'customer.subscription.updated': {
      ...
      if (!customerId) {
        await reportStripeWebhookFailure('subscription.updated_missing_customer', {
          stripe_event_id: event.id,
      ...
      if (error) {
        await reportStripeWebhookFailure('subscription.updated_supabase_error', {
          stripe_event_id: event.id,
          subscription_id: sub.id,
          customer_id: customerId,
        }, error)
      ...
      if (!rows?.length) {
        await reportStripeWebhookFailure('subscription.updated_no_profile_row', {
          stripe_event_id: event.id,
          subscription_id: sub.id,
          customer_id: customerId,
    ...
    case 'customer.subscription.deleted': {
      ...
      const customerId = typeof sub.customer === 'string' ? sub.customer : sub.customer?.id
      if (!customerId) {
        await reportStripeWebhookFailure('subscription.deleted_missing_customer', {
          stripe_event_id: event.id,
          subscription_id: sub.id,
        })
        return
      ...
      if (error) {
        await reportStripeWebhookFailure('subscription.deleted_supabase_error', {
          stripe_event_id: event.id,
          subscription_id: sub.id,
          customer_id: customerId,
        }, error)
      ...
      if (!rows?.length) {
        await reportStripeWebhookFailure('subscription.deleted_no_profile_row', {
          stripe_event_id: event.id,
          subscription_id: sub.id,
          customer_id: customerId,
        }, new Error('No user_profiles row matched'))
