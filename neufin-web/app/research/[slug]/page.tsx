...
function regimeBadge(regime: string | null | undefined) {
  ...
  return m[k] || regime.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())
}

function regimeBadgeClass(regime: string | null | undefined) {
  const k = (regime || '').toLowerCase()
  if (k.includes('risk_on') || k === 'recovery') return 'border-emerald-500/40 bg-emerald-500/15 text-emerald-200'
  if (k.includes('risk_off') || k.includes('recession')) return 'border-red-500/40 bg-red-500/15 text-red-200'
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 px-6 py-section lg:grid-cols-[minmax(0,1fr)_300px]">
        <article>
          ...
          <p className="mt-8 text-sm text-muted-foreground">
            ...
            {new Date(note.created_at).toLocaleString('en-SG', { dateStyle: 'medium', timeStyle: 'short' })}. Based on{' '}
            {typeof note.macro_signal_count === 'number' ? note.macro_signal_count : '—'} macro signals.
          </p>

          <div className="mt-10 rounded-xl border border-primary/30 bg-primary/10 p-6 text-center">
            <p className="text-sm text-muted-foreground">
              Try NeuFin free for 14 days — full portfolio DNA, IC-ready reports, and swarm intelligence.
            </p>
            <Link
              href="/upload"
        ...
        <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start">
          <div className="rounded-xl border border-border bg-surface p-4">
            ...
            <p className="text-sm font-medium text-foreground">{regimeBadge(note.regime)}</p>
            <p className="mt-1 text-xs text-muted-foreground">
              Confidence:{' '}
              {Math.round((note.confidence_score ?? 0) <= 1 ? (note.confidence_score ?? 0) * 100 : (note.confidence_score ?? 0))}%
            </p>
          </div>
