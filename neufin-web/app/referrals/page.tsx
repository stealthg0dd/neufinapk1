...
const MILESTONES = [
  { count: 1, icon: "🎟️", reward: "20% off your next report", color: "blue" },
  {
    count: 3,
    icon: "📄",
    reward: "1 free Pro Report ($29 value)",
    color: "purple",
  },
...
export default function ReferralsPage() {
  ...
  const activeMilestone =
    [...MILESTONES].reverse().find((m) => shareCount >= m.count) ?? null;
  const nextMilestone = MILESTONES.find((m) => shareCount < m.count) ?? null;
  const progressPct = Math.min(
    100,
    Math.round((shareCount / MAX_MILESTONE) * 100),
  );
  ...
  const shareTwitter = () => {
    ...
    const score = (() => {
      try {
        return JSON.parse(localStorage.getItem("dnaResult")!).dna_score;
      } catch {
        return "?";
      }
    })();
    ...
    window.open(
      `https://twitter.com/intent/tweet?text=${encodeURIComponent(text + referralUrl)}`,
      "_blank",
    );
  ...
  const shareWhatsApp = () => {
    if (!referralUrl) return;
    window.open(
      `https://wa.me/?text=${encodeURIComponent(`Check out my Investor DNA Score 🧬 — use my link and get 20% off your first report: ${referralUrl}`)}`,
      "_blank",
    );
      ...
      <div className="min-h-screen flex flex-col bg-shell-deep">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 p-6 text-center">
          ...
          <h1 className="text-xl font-bold text-white">
            Run your DNA analysis first
          </h1>
          <p className="text-shell-subtle text-sm max-w-xs">
            Your unique referral link is generated after your first portfolio
            scan.
          </p>
            ...
            {referralUrl ? (
              <>
                ...
                <p className="text-xs text-shell-subtle mt-3 text-center">
                  Anyone who uses your link gets 20% off their first report
                  automatically
                </p>
              </>
            ) : (
          ...
          <motion.div variants={fadeUp} className="card">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-shell-subtle">
                {shareCount} / {MAX_MILESTONE} shares
              </span>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-sm font-semibold ${unlocked ? colors.text : "text-shell-muted"}`}
                      >
                        {m.reward}
                      </p>
                      <p className="text-xs text-shell-subtle">
                        {unlocked
                          ? "Unlocked!"
                          : `Share with ${m.count} friend${m.count > 1 ? "s" : ""}`}
                      </p>
                    </div>
                    {unlocked && (
                      <AnimatePresence>
            ...
            {nextMilestone && (
              <p className="text-center text-xs text-shell-subtle mt-4">
                {nextMilestone.count - shareCount} more share
                {nextMilestone.count - shareCount > 1 ? "s" : ""} to
                unlock:&nbsp;
                <span className="text-shell-muted font-medium">
              ...
              {[
                {
                  ...
                  body: "Send your unique referral link to friends via any channel.",
                },
                {
                  step: "2",
                  title: "Friend uploads CSV",
                  body: "They analyze their portfolio — no purchase required to activate your discount.",
                },
                {
                  step: "3",
                  title: "They get 20% off",
                  body: "When they unlock a Pro Report, the discount applies automatically.",
                },
                {
                  step: "4",
                  title: "You unlock rewards",
                  body: "Hit milestones to earn free reports and lifetime pro access.",
                },
              ].map(({ step, title, body }) => (
                <div key={step} className="flex gap-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-shell-fg">
                      {title}
                    </p>
          ...
          {activeMilestone && (
            ...
            >
              ...
              <p className="text-xs text-shell-subtle mt-1">
                Email us at{" "}
                <span className="text-shell-muted">support@neufin.app</span> to
                claim your reward.
              </p>
            </motion.div>
          )}
