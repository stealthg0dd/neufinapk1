...
function isValidMetricValue(v: string | number | null | undefined): boolean {
  ...
  if (typeof v === 'string') {
    const t = v.trim()
    if (!t || t === '—' || t.toLowerCase() === 'nan' || t.toLowerCase() === 'n/a') return false
  }
...
function CoverBlock({ model }: { model: InstitutionalReportModel }): ReactNode {
  ...
  return (
    <section className="report-cover" aria-label="Cover">
      <p className="report-cover-eyebrow">Confidential · Investment committee</p>
    ...
    <section key={sec.id} className="report-section" id={sec.id}>
      <header className="report-section-header">
        ...
        <div className="report-section-titles">
          <h2>{sec.title}</h2>
          <span className="report-section-confidence" data-level={sec.confidence}>
  ...
  return (
    <article className="neufin-report">
      ...
      <footer className="report-footer">
        Confidential — professional use only. Not investment advice. Reconcile all figures against
        custodian statements before committee presentation.
      </footer>
    </article>
  )
}
  ...
  return {
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: ['Review single-name concentration vs. policy limits.', 'Stress paths summarized in Scenario analysis.'],
      },
      {
        id: 'behavioral',
      ...
      },
      {
        ...
        bullets: ['Rebalance toward policy weights.', 'Document rationale for any tactical overrides.'],
      },
      {
        id: 'appendix',
        title: 'Appendix',
        confidence: 'high',
        status: 'complete',
        summary: 'Methodology, data sources, and limitations. Retain with the committee pack for audit trail.',
        metrics: {
