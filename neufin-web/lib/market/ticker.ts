...
const SYMBOLS = ['AAPL', 'GOOGL', 'MSFT', 'TSLA', 'NVDA', 'META', 'AMZN', 'NFLX', 'BABA', 'BRK.B', 'JPM', 'GS'] as const
...
async function fetchGlobalQuote(symbol: string, apiKey: string): Promise<TickerItem | null> {
  ...
  try {
    ...
    const quote = (data['Global Quote'] ?? data['Global quote'] ?? null) as Record<string, unknown> | null
...
export async function getLandingTickers(): Promise<TickerItem[]> {
  ...
  const settled = await Promise.allSettled(SYMBOLS.map((s) => fetchGlobalQuote(s, apiKey)))
  const items = settled
  ...
  return SYMBOLS.map((s) => bySymbol.get(s.toUpperCase()) ?? MOCK.find((m) => m.symbol === s)!).filter(Boolean)
}
